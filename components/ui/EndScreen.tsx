...
const EndScreen: React.FC<EndScreenProps> = ({ status, onRestart, deathCount }) => {
  const messages = {
    gameOver: {
  ...
  const subtitle = status === 'gameOver' && deathCount !== undefined
  ...
  return (
    <div className="text-center animate-fadeIn flex flex-col items-center justify-center">
      ...
      <p className="mt-4 text-2xl text-gray-300">{subtitle}</p>
      <button
        onClick={onRestart}
